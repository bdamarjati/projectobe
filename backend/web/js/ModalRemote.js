...
function ModalRemote(modalId) {
  ...
  this.loadingContent =
    '<div class="progress"><div class="progress-bar progress-bar-striped bg-primary progress-bar-animated" role="progressbar" style="width: 100%" aria-valuenow="100" aria-valuemin="0" aria-valuemax="100"></div></div>';
  ...
  this.clear = function() {
    ...
    colElm.setAttribute("class", "col-12 text-right");

    $(this.footer).append(colElm);
  };
  ...
  function errorRemoteResponse(response) {
    if (response.status != 302) {
      ...
      this.setContent("<pre>" + response.responseText + "</pre>");
      this.addFooterButton("Batal", "button", "btn btn-secondary", function(
        button,
        event
      ) {
        this.hide();
